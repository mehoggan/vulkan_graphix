// Fault-injects Tutorial07's own create*() failure branches - see
...
TEST(Tutorial07FaultInjectionTest,
    ...
    ASSERT_TRUE(window.create("fault-injection-tutorial07"));

    vulkan_graphix::Tutorial07 tutorial;
    ...
    // createStagingBuffer()/createVertexBuffer()/createUniformBuffer() all
    // route through the shared createBuffer() helper, whose first call is
    // vkCreateBuffer - fault it once per public entry point, since
    // coverage tracks each call site's own `if (!createBuffer(...))` check.
    {
