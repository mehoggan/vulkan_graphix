// Fault-injects Tutorial09's own create*() failure branches - see
...
TEST(Tutorial09FaultInjectionTest,
    ...
    ASSERT_TRUE(window.create("fault-injection-tutorial09"));

    vulkan_graphix::Tutorial09 tutorial;
    ...
    // createStagingBuffer()/createUniformBuffer()/createVertexBuffer()/
    // createIndexBuffer() all route through the shared createBuffer()
    // helper, whose first call is vkCreateBuffer - fault it once per
    // public entry point, since coverage tracks each call site's own
    // `if (!createBuffer(...))` check.
    {
        ...
        EXPECT_FALSE(tutorial.createIndexBuffer());
        vulkan_graphix::vkCreateBuffer = real_fn;
    }
    ASSERT_TRUE(tutorial.createStagingBuffer());

    // createDepthResources() and createTexture() both route through the
    // shared createImage() helper, whose first call is vkCreateImage -
    // fault it once per public entry point.
    {
