...
bool VBOShaderLibrary::loadShaders(const char* vsFileName, const char* fsFileName) {
	...
	cout << "Going to read " << vsFileName << endl;
	FILE* vs_file;
	...
	cout << "Going to read " << fsFileName << endl;
	FILE* fs_file;
...
void VBOShaderLibrary::LoadTextureNormals(const char* filename, int width, int height) {
    ...
    file = fopen(filename, "rb");
	if(!file) {
		cout << "ERROR: File Does Not Exist Not Going to Use Texture Normals" << endl;
	}else {
		data=new unsigned char[width*height*3];
		fread(data,width*height*3,1,file);
		fclose(file);
...
bool VBOShaderLibrary::AreVBOsSupported() {
	...
	if(str){
		...
		extensions=new char*[extensions_supported];			// <--------- THE LEAK!
	}else {
		qualified = false;
	}
