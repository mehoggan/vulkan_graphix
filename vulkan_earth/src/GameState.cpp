...
void GameState::drawHUD(){
	...
	for(int i=0;i<player_factory->getNumberofPlayers();i++){
		...
		char buffer[128];
 		memset(buffer,0,128);
		drawHUDText((sprintf(buffer,"%d",(int)player_factory->getPlayer(i)->getCurrentWait()),buffer),-0.95*newX,0.70*newY-0.05*newY*i);
		...
		drawHUDText((sprintf(buffer,"%d",(int)player_factory->getPlayer(i)->getCurrentTank()->getHP()),buffer),-0.87*newX,0.70*newY-0.05*newY*i);
	}
	...
	if(this->gameSubState==PLAYER_CONTROL){
		...
		drawHUDText((sprintf(buffer,"%d",(int)(powerRatio*1000)),buffer),0.95*newX,-0.7*newY);
		// show previous power
		char buffer1[128];
 		memset(buffer1,0,128);
		glColor3f(0.55,0.55,0.55);
		drawHUDText("Power:",0.7*newX,-0.65*newY);
		drawHUDText((sprintf(buffer1,"%d",player_factory->getPlayer(currentPlayerIndex)->getCurrentTank()->getPreviousPower()),buffer1),
							0.95*newX,-0.65*newY);
		...
		drawHUDText((sprintf(buffer2,"%d",(int)(player_factory->getPlayer(currentPlayerIndex)->getCurrentTank()->getTurretDegrees()+1)),buffer2),
							0.95*newX,-0.8*newY);
		// show previous angle
		char buffer3[128];
		...
		drawHUDText((sprintf(buffer3,"%d",player_factory->getPlayer(currentPlayerIndex)->getCurrentTank()->getPreviousAngle()),buffer3),
							0.95*newX,-0.75*newY);
