...
VkRenderPass& VulkanTutorial04Parameters::getVkRenderPass() {
    return m_vk_render_pass;
}
void VulkanTutorial04Parameters::setVkRenderPass(
        const VkRenderPass& vk_render_pass) {
    m_vk_render_pass = vk_render_pass;
}

const VkPipeline& VulkanTutorial04Parameters::getVkGraphicsPipeline() const {
    return m_vk_graphics_pipeline;
}
VkPipeline& VulkanTutorial04Parameters::getVkGraphicsPipeline() {
    return m_vk_graphics_pipeline;
}
void VulkanTutorial04Parameters::setVkGraphicsPipeline(
        const VkPipeline& vk_graphics_pipeline) {
    m_vk_graphics_pipeline = vk_graphics_pipeline;
}

const BufferParameters& VulkanTutorial04Parameters::getVertexBufferParameters()
        const {
    return m_vertex_buffer;
}
BufferParameters& VulkanTutorial04Parameters::getVertexBufferParameters() {
    return m_vertex_buffer;
}
void VulkanTutorial04Parameters::setVertexBufferParameters(
        const BufferParameters& vertex_buffer) {
    m_vertex_buffer = vertex_buffer;
}

const VkCommandPool& VulkanTutorial04Parameters::getVkCommandPool() const {
    return m_vk_command_pool;
}
VkCommandPool& VulkanTutorial04Parameters::getVkCommandPool() {
    return m_vk_command_pool;
}
void VulkanTutorial04Parameters::setVkCommandPool(
        const VkCommandPool& vk_command_pool) {
    m_vk_command_pool = vk_command_pool;
}

const std::vector<RenderingResourceParameters>&
VulkanTutorial04Parameters::getRenderingResources() const {
    return m_rendering_resources;
}
std::vector<RenderingResourceParameters>&
VulkanTutorial04Parameters::getRenderingResources() {
    return m_rendering_resources;
}
void VulkanTutorial04Parameters::setRenderingResources(
...
VulkanTutorial04Parameters::getFinishedRenderingSemaphores() const {
    return m_finished_rendering_semaphores;
}
std::vector<VkSemaphore>&
VulkanTutorial04Parameters::getFinishedRenderingSemaphores() {
    return m_finished_rendering_semaphores;
}
void VulkanTutorial04Parameters::setFinishedRenderingSemaphores(
...
Tutorial04::createPipelineLayout() {
...
bool Tutorial04::createPipeline() {
    Tools::AutoDeleter<VkShaderModule, PFN_vkDestroyShaderModule>
            vertex_shader_module = createShaderModule("shader.04.vert.spv");
    Tools::AutoDeleter<VkShaderModule, PFN_vkDestroyShaderModule>
            fragment_shader_module = createShaderModule("shader.04.frag.spv");
...
bool Tutorial04::createVertexBuffer() {
    ...
    VkMappedMemoryRange flush_range = {
            ...
            .pNext = nullptr,
            .memory = vertex_buffer.getVkDeviceMemory(),
            .offset = 0,
            .size = VK_WHOLE_SIZE};
...
bool Tutorial04::createSemaphores() {
    ...
    // swapchain image, not by the rendering-resource slot: the fence above
    ...
    std::vector<VkSemaphore>& finished_rendering_semaphores =
            m_vulkan_tutorial04_parameters.getFinishedRenderingSemaphores();
...
bool Tutorial04::createFramebuffer(VkFramebuffer& framebuffer,
...
bool Tutorial04::draw() {
    ...
    RenderingResourceParameters& current_rendering_resource =
            m_vulkan_tutorial04_parameters
...
void Tutorial04::childClear() {
    ...
    BufferParameters& vertex_buffer =
            m_vulkan_tutorial04_parameters.getVertexBufferParameters();
    if (vertex_buffer.getVkBuffer() != VK_NULL_HANDLE) {
        vkDestroyBuffer(getVkDevice(), vertex_buffer.getVkBuffer(), nullptr);
        vertex_buffer.setVkBuffer(VK_NULL_HANDLE);
    }
    if (vertex_buffer.getVkDeviceMemory() != VK_NULL_HANDLE) {
        vkFreeMemory(
                getVkDevice(), vertex_buffer.getVkDeviceMemory(), nullptr);
        vertex_buffer.setVkDeviceMemory(VK_NULL_HANDLE);
    }
